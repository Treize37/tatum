  ...
  class ApproveNftSpendingKMS
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::ApproveNftSpendingKMS` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::ApproveNftSpendingKMS`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @is_erc721.nil?
        invalid_properties.push('invalid value for "is_erc721", is_erc721 cannot be nil.')
      end
    ...
    def chain=(chain)
      validator = EnumAttributeValidator.new('Object', ['ETH', 'ONE', 'BSC', 'MATIC'])
    ...
    def hash
      [chain, contract_address, spender, is_erc721, token_id, signature_id, index, nonce, fee].hash
    end
