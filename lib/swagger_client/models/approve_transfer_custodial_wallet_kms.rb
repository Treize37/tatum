...
module SwaggerClient
  class ApproveTransferCustodialWalletKMS
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::ApproveTransferCustodialWalletKMS` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::ApproveTransferCustodialWalletKMS`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'spender')
        self.spender = attributes[:'spender']
      end
    ...
    def list_invalid_properties
      ...
      if @spender.nil?
        invalid_properties.push('invalid value for "spender", spender cannot be nil.')
      end
    ...
    def valid?
      ...
      contract_type_validator = EnumAttributeValidator.new('Object', ['0', '1', '2'])
      return false unless contract_type_validator.valid?(@contract_type)
      return false if @spender.nil?
    ...
    def hash
      [chain, custodial_address, token_address, contract_type, spender, amount, token_id, signature_id, index, nonce, fee].hash
    end
