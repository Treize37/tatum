  ...
  class GenerateAuctionKMS
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::GenerateAuctionKMS` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::GenerateAuctionKMS`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'auction_fee')
        self.auction_fee = attributes[:'auction_fee']
      end
    ...
    def list_invalid_properties
      ...
      if @auction_fee.nil?
        invalid_properties.push('invalid value for "auction_fee", auction_fee cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @auction_fee.nil?
    ...
    def hash
      [chain, fee_recipient, auction_fee, signature_id, index, nonce, fee].hash
    end
