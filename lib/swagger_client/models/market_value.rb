  ...
  class MarketValue
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::MarketValue` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::MarketValue`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'source_date')
        self.source_date = attributes[:'source_date']
      end
    ...
    def list_invalid_properties
      ...
      if @source_date.nil?
        invalid_properties.push('invalid value for "source_date", source_date cannot be nil.')
      end
    ...
    def hash
      [amount, currency, source_date, source].hash
    end
