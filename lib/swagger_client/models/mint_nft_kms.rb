  ...
  class MintNftKMS
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::MintNftKMS` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::MintNftKMS`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'erc20')
        self.erc20 = attributes[:'erc20']
      end
      ...
      if attributes.key?(:'fixed_values')
        if (value = attributes[:'fixed_values']).is_a?(Array)
          self.fixed_values = value
        end
      end
    ...
    def hash
      [chain, token_id, to, contract_address, erc20, url, provenance, author_addresses, cashback_values, fixed_values, index, signature_id, nonce, fee].hash
    end
