...
module SwaggerClient
  class SellAssetOnMarketplaceKMS
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::SellAssetOnMarketplaceKMS` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::SellAssetOnMarketplaceKMS`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @nft_address.nil?
        invalid_properties.push('invalid value for "nft_address", nft_address cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @nft_address.nil?
    ...
    def hash
      [chain, contract_address, nft_address, seller, erc20_address, listing_id, amount, token_id, price, is_erc721, signature_id, index, nonce, fee].hash
    end
