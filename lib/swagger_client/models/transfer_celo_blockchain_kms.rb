...
module SwaggerClient
  class TransferCeloBlockchainKMS
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::TransferCeloBlockchainKMS` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::TransferCeloBlockchainKMS`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      currency_validator = EnumAttributeValidator.new('Object', ['CELO', 'CUSD', 'CEUR'])
    ...
    def hash
      [data, nonce, currency, to, fee_currency, amount, index, signature_id].hash
    end
