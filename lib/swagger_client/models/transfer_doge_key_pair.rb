  ...
  class TransferDogeKeyPair
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::TransferDogeKeyPair` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::TransferDogeKeyPair`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'key_pair')
        if (value = attributes[:'key_pair']).is_a?(Array)
          self.key_pair = value
        end
      end
    ...
    def list_invalid_properties
      ...
      if @key_pair.nil?
        invalid_properties.push('invalid value for "key_pair", key_pair cannot be nil.')
      end
    ...
    def hash
      [sender_account_id, address, amount, compliant, fee, multiple_amounts, key_pair, attr, payment_id, sender_note].hash
    end
