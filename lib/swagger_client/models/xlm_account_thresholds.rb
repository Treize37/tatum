  ...
  class XlmAccountThresholds
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::XlmAccountThresholds` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::XlmAccountThresholds`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'low_threshold')
        self.low_threshold = attributes[:'low_threshold']
      end

      if attributes.key?(:'med_threshold')
        self.med_threshold = attributes[:'med_threshold']
      end

      if attributes.key?(:'high_threshold')
        self.high_threshold = attributes[:'high_threshold']
      end
    end
    ...
    def hash
      [low_threshold, med_threshold, high_threshold].hash
    end
