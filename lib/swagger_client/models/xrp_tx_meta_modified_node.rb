...
module SwaggerClient
  class XrpTxMetaModifiedNode
    attr_accessor :final_fields
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `SwaggerClient::XrpTxMetaModifiedNode` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `SwaggerClient::XrpTxMetaModifiedNode`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'final_fields')
        self.final_fields = attributes[:'final_fields']
      end
      ...
      if attributes.key?(:'previous_fields')
        self.previous_fields = attributes[:'previous_fields']
      end
    ...
    def hash
      [final_fields, ledger_entry_type, ledger_index, previous_fields, previous_txn_id, previous_txn_lgr_seq].hash
    end
